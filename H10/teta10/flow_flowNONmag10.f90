...
double precision :: flow_angle,flow_angle1,rrough,deltarough,saveme,Hmax,ktotal,fi
...
!- Ajustar o ângulo antes de simular
 ...
 flow_angle1 = (10.d0)*0.01745329252d0
...
!Para gerar o arquivo da condiçao inicial
 Kvw = 1.d0
 call salva_eps(cont,Lx,Ly,Nballs,R,S(:,1),S(:,2),ang,whosmag,Nroughs,rrough,Srough,Kvw,scale,xinfesq,yinfesq,xsupdir,ysupdir)

!Loop para correr o tempo
do n=ninit,60*100000
	Cell = -1
	marcabola = -1
...
& ,Kvw,scale,xinfesq,yinfesq,xsupdir,ysupdir)
 implicit none
 ...
 double precision, dimension (1:parts), intent (in) :: Kvw
 ...
 do i=1,parts
	if(whosmag(i).eq.1) then !Para desenhar as bolinhas magnéticas
		...
		if((part_pos_x(i)-part_raio(i)).lt.0.d0) then	
			...
			write(210,*) 'stroke'
			write(210,101) (Kvw(i)/maxK), 0.d0, 1.d0 - (Kvw(i)/maxK) !Colocar aqui a parada da velocidade
...
subroutine salva_velo(ttwrite,nxis,nyip,vperfil)
 ...
 if(ttwrite.ne.0) then
	filename1="vper.000.dat"
    filename1(8:8)=CHAR(48+mod(ttwrite,100)-10*(mod(ttwrite,100)/10))
    filename1(7:7)=CHAR(48+(mod(ttwrite,100)/10))
    filename1(6:6)=CHAR(48+ttwrite/100)
 else
	filename1="vper.000.dat"
 end if
