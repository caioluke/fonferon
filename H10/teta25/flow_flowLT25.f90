...
double precision, dimension (:,:,:), allocatable :: Fnormal,vperfil,oldz
double precision, dimension (:,:), allocatable :: FR,Fparede,Fatrito,Froughness,oldzr
...
open(unit=31,file='ktotalH10T25.dat',status='unknown')
...
allocate(oldz(Nballs,Nballs,3))
allocate(oldzr(Nballs,3))
...
!- Ajustar o ângulo antes de simular
 ...
 flow_angle1 = (25.d0)*0.01745329252d0
...
do n=ninit,100*10000
	...
	Froughness = 0.d0
	Torque = 0.d0
	FR(:,1) = -m(:)*0.1d0*sin(flow_angle)
	FR(:,2) = -m(:)*0.1d0*cos(flow_angle)
					...
					do while (ultbola.ne.-1) !While para mudar a última bola					
						...
						do while (penbola.ne.-1) !While para perguntar entre as bolas de índices menores
							...
							if(norm2(D).lt.(R(j)+R(i))) then !Força de contato = ...
								call contact_force(Nballs,j,i,K,gamaS,gamaN1,minormal,oldz,D,v,omega,R,Torque,Fatrito,Fnormal,FR)
							end if
						...
						do while (ultbola.ne.-1) !While para mudar a bola dentro da célula escolhida
							...
							do while (penbola.ne.-1) !While para perguntar entre as rugosidades de índices menores
								...
								if(norm2(D).lt.(R(j)+rrough)) then !Força de contato = ...
									call roughness_force(Nballs,j,rrough,K,gamaS,gamaN2,minormal,oldzr,D,v,omega,R,Torque,Fatrito,Froughness,FR)
								end if 
...
subroutine contact_force(Nballs,j,i,K,gamaS,gamaN1,minormal,oldz,D,v,omega,R,Torque,Fatrito,Fnormal,FR)
 implicit none
 ...
 double precision, dimension (Nballs,Nballs,3), intent (inout) :: oldz

 double precision, dimension (2) :: csi,n_dir,s_dir,vrel,vtan,z,f
 ...
 Fnormal(i,j,:) = K*csi(:) - 0.5d0*gamaN1*vrelnormal*n_dir(:)
 Fnormal(j,i,:) = -Fnormal(i,j,:)

 !50 passos de colisão, mas como começa do 0: 0 ~ 49, ou seja, se passar de 49 tem que zerar
 if(oldz(j,i,3).gt.49.9) then
	z = 0.d0
	oldz(j,i,3) = 0.d0
	oldz(i,j,3) = oldz(j,i,3)
 else
	z(1) = oldz(j,i,1)
	z(2) = oldz(j,i,2)
	oldz(j,i,3) = oldz(j,i,3) + 1.d0
	oldz(i,j,3) = oldz(j,i,3)
 end if
 
 vtan(:) = vreltangente*s_dir(:)
 
 f(1) = vtan(1) - (z(1)*vrel(1))*D(1)/norm2(D(:))
 f(2) = vtan(2) - (z(2)*vrel(2))*D(2)/norm2(D(:))
 z(:) = z(:) + ((10.d0)**(-4.d0))*f(:)
 
 oldz(j,i,1) = z(1)
 oldz(j,i,2) = z(2)
 oldz(i,j,1) = oldz(j,i,1)
 oldz(i,j,2) = oldz(j,i,2)
 
 Fatrito(j,:) = -(2.d0*K/7.d0)*z(:)
 Fatrito(i,:) = - Fatrito(j,:)
 FR(j,:) = FR(j,:) + Fatrito(j,:)
 FR(i,:) = FR(i,:) + Fatrito(i,:)
...
subroutine roughness_force(Nballs,j,rrough,K,gamaS,gamaN2,miparede,oldzr,D,v,omega,R,Torque,Fatrito,Froughness,FR)
 implicit none
 ...
 double precision, dimension (Nballs,3), intent (inout) :: oldzr
 ...
 !50 passos de colisão, mas como começa do 0: 0 ~ 49, ou seja, se passar de 49 tem que zerar
 if(oldzr(j,3).gt.49.9) then
	z = 0.d0
	oldzr(j,3) = 0.d0
 else
	z(1) = oldzr(j,1)
	z(2) = oldzr(j,2)
	oldzr(j,3) = oldzr(j,3) + 1.d0
 end if
