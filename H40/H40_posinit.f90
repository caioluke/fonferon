...
double precision :: rrough,deltarough,saveme,Hmax
...
!- A divisão de Lx/(2*rrough+deltarough) tem que ser inteira
! Lx tem que ser divisivel por 2*Lcell se rrough = 2.d0*rmax = Lcell
...
!Dados que precisam satisfazer condições especiais
 ...
 Lx = 50.d0*Lcell                             !Tamanho da parede em x (m)
 Ly = (40.d0 + 2.d0)*Lcell                    !Tamanho da parede em y (m)
!*********************************************************************************
...
!Número de rugosidades
 ...
 !Número de bolas
 !Tirei as células da rugosidade, se quiser, pode tirar mais células também
 !Basta somar lá no Ly
 Nballs = (nxis)*(nyip-nyiprough)
...
!Para gerar o arquivo da condiçao inicial
 cont = 0
 call salva_eps(cont,Lx,Ly,Nballs,R,S(:,1),S(:,2),ang,Nroughs,rrough,Srough,scale,0.d0)
...
!Loop para correr o tempo
 do n=1,2*100000
	...
	if(mod(n,10000).eq.0) then
		cont = cont + 1
		tempogif = n*h
		call salva_eps(cont,Lx,Ly,Nballs,R,S(:,1),S(:,2),ang,Nroughs,rrough,Srough,scale,tempogif)
	end if
...
open(unit=51,file='initflow.dat',status='unknown')
...
subroutine pos_init(S,v,teta,omega,ang,R,m,Inercia,densidade,rmax,Lcell,nxis,nyip,nyiprough,Nballs)
 ...
 integer :: i,j,initrandom,bola,xis,yip,mag,nloop
...
& ,scale,tempogif)
 ...
 write(210,94)
 write(210,95)
 write(210,96) 0,0,int(scale*Lx)+1,int(scale*Ly)
 write(210,97)
 write(210,98)
 write(210,99)
 write(210,100)
 ...
 write(210,105) scale*0.15d0*Lx,scale*0.9d0*Ly
