program colSphN
	...
	double precision :: h, K, t, CsiW, Room, Circumradius
	...
	open(unit=129, file='div.dat', status='unknown')
	open(unit=120, file='conv.dat', status='unknown')
	...
	do i = 1, 360
		...
		Itheta(1) = 0.d0
		Itheta(2) = Itheta(2) + (3.141596d0/180.d0)
		S(:,2) = Sa(:,2) + h*V(:,2)
		Theta(2) = ThetaA(2) + h*W(2)	
		...
		do j = 1, 360
			...
			call salva_eps(0,N,R(:),S(1,:),S(2,:),Theta(:))
			...
			Itheta(1) = Itheta(1) + (3.141596d0/180.d0)
