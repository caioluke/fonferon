...
double precision, dimension (:,:,:), allocatable :: Fnormal,Fmag
...
integer, dimension (:,:), allocatable :: marcabola,marcarough,marcabolamag
...
double precision :: rrough,deltarough,saveme,Hmax,mi0
...
real :: start,finish,days,hours,mins,secs
call cpu_time(start)
...
!Dados que precisam satisfazer condições especiais
 ...
 rmax = 0.011111d0                            !Maior raio das bolinhas (m)
...
!*********************************************************************************
 ...
 !Coeficiente de restituição = 0.78
 gamaN1 = 6.4507d0                            !Coeficiente de dissipação entre as bolinhas
 gamaN2 = 6.3478d0                            !Coeficiente de dissipação entre a bola e a parede/rugosidade
 gamaS = 25.d0                                !Coeficiente da força tangente, protege para o fat não dar pau
 minormal = 0.5d0                             !Coeficiente de atrito entre as bolinhas
 miparede = 0.5d0                             !Coeficiente de atrito da parede
 mi0 = 1.25664d0*(10.0**(-6.d0))              !Permissividade do meio
!*********************************************************************************
...
!Subrotina da posiçao inicial, escolhe quais bolas são magnéticas
 call pos_init(S,v,teta,omega,ang,R,m,Inercia,densidade,rmax,Lcell,nxis,nyip,nyiprough,Nballs,Nmags,whosmag)
 mdipolo(:,1) = cos(teta(:))
 mdipolo(:,2) = sin(teta(:))
...
!Para gerar o arquivo da condiçao inicial
 cont = 0
 call salva_eps(cont,Lx,Ly,Nballs,R,S(:,1),S(:,2),ang,whosmag,Nroughs,rrough,Srough,scale,xinfesq,yinfesq,xsupdir,ysupdir)
...
subroutine pos_init(S,v,teta,omega,ang,R,m,Inercia,densidade,rmax,Lcell,nxis,nyip,nyiprough,Nballs,Nmags,whosmag)
 implicit none
 ...
 integer, dimension (Nballs), intent (inout) :: whosmag
 integer, intent (in) :: Nballs,nxis,nyip,nyiprough,Nmags
 
 integer :: i,j,initrandom,bola,xis,yip,mag,nloop,extra
 ...
 do nloop=1,Nmags+extra
	mag = 0
	do while(mag.eq.0)
		mag = int(Nballs*rand(0))
	end do
	
	if(whosmag(mag).eq.1) then
		extra = extra + 1
	else
		whosmag(mag) = 1
	end if	
 ...
 do yip= nyiprough+1,nyip
	do xis= 0,nxis-1
		...
		if (bola.le.Nballs) then		
			S(bola,1) = Lcell*( ((0.5d0 + 1.0d0*xis)) + 0.045d0*(rand(0)-0.5d0) )
			S(bola,2) = Lcell*( ((0.5d0 + 1.0d0*yip)) + 0.045d0*(rand(0)-0.5d0) )
			R(bola)   = rmax*(0.9d0 + (rand(0)-0.5d0)*0.1d0)			
		end if
